@Story
  Feature: The user wants to amend an exising project using a POST call
    ...
    # Normal Flow
    Scenario Outline: User wants to amend all fields of an existing project in the database using a POST call
      Given An already existing project with a title <tit>, a description <desc>, an active status <act> and a completed status <comp>
      When The user tries to amend the project with a new title <newtit>, a new description <newdesc>, a new active status <newact> and a new completed status <newcomp> using POST
    ...
    # Alternate Flow
    Scenario Outline: User wants to amend no fields of an existing project in the database using a POST call
      Given An already existing project with a title <tit>, a description <desc>, an active status <act> and a completed status <comp>
    ...
    # Error Flow
    Scenario Outline: User wants to amend the id of an existing project in the database using a POST call
      Given An already existing project with a title <tit>, a description <desc>, an active status <act> and a completed status <comp>
